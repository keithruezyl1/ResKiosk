...
| D9 | Metrics logging location | ✅ **resolved** — (a) **extend `query_logs`** with additive columns (latency breakdown `retrieve_ms`/`rewrite_ms`/`clarification_ms`; grounding `grounded_ratio`/`unsupported_span_count`/`grounding_method`/`grounding_detail`) + compute KPIs **on read** (no metrics table). (b) Hallucination proxy = **rule-based lexical support, offline** (`grounding_method="rule_v1"`; optional offline LLM-judge later, never hot path). (c) Privacy = **structured-only exports** + `RESKIOSK_QUERY_TEXT_RETENTION_DAYS` (**default 30**) purge helper that NULLs transcripts past the window. Deferred: modality dim (Phase 7), normalized per-span table. | Phase 5 |
| D10 | Config-version signal for cache | ✅ **resolved** — **computed `config_signature`** (hash of retrieval-affecting config: sim/clarification thresholds, RRF_K, HYBRID_TOP_K, compound thresholds, RLHF flags, taxonomy/filter policy files) combined with `kb_version` in the cache key. No new version column; config change → restart → new hash → old keys miss. | Phase 6 |
| D11 | Cache safety scope | ✅ **resolved** — **bypass cache entirely for safety-critical intents** (safety, emergency, medical, children, special_needs); log `cache_status=bypass`. **Single-flight/coalescing deferred** (single-process, low QPS). Re-validation deferred. | Phase 6 |
| D12 | KB table strategy | ✅ **resolved** — replace `kb_articles` with a canonical multimodal **`kb_items`** table (modality, image_asset_id, parent_article_id, segment_index + existing fields); **no compatibility view** (SQLite views are read-only, conflicts with ORM writes); **wipe & re-seed dev data** (still in development). `source_id` stays `kb_items.id` (int) so logs/feedback/FAQ are unaffected. An image is a `kb_items` row (`modality='image'`) referencing its binary via `image_asset_id` → `kb_assets` (7B): one ID space. | Phase 7 |
| D13 | Metadata storage form | ✅ **resolved** — **typed columns** for filtered/retrieved fields (modality, image_asset_id, parent_article_id, segment_index, authority/scope/center_id/hub_id) + **one nullable JSON column** for open-ended extras (mime/dims/captions later). Evidence contract: keep `source_id`; **add `modality`** (+ nullable render/asset ref) to QueryResponse/SecondaryEvidence/final_evidence; top_k_ids stay plain id lists. | Phase 7 |
| D14 | Image binary storage | ✅ **resolved** — **filesystem** under `<db_dir>/assets/` (`RESKIOSK_ASSETS_DIR` override; reuses the proven-writable per-user dir, EXE-safe), DB stores refs not bytes. **Content-addressed** sha256 names, sharded `assets/<hash[:2]>/<hash>.<ext>`, deterministic variants (`_thumb`/`_disp`) → free dedupe + integrity. `kb_assets` row keeps a stable int id (← `kb_items.image_asset_id`) + content_hash + mime/size + refs + kb_version + 4-state status. Served via **hub endpoint `GET /assets/{id}/{variant}`** (thumb\|display\|original), gated to `ready`/published; `render_ref` = that URL. | Phases 7, 8 |
...
| D16 | Image model choice | ✅ **resolved** — **CLIP ViT-B/32 via sentence-transformers** (`clip-ViT-B-32`): shared image+text space, **Apache-2.0**, CPU, drops into the existing `embedder.py` load pattern (bundled dir, `local_files_only`). Deterministic with fixed preprocessing; encode image vectors at ingest/publish. NOTE: CLIP space ≠ MiniLM text space → image retrieval is a **separate path** (CLIP text-query vs CLIP image vectors) merged with text results. SigLIP (also Apache-2.0, better zero-shot) is the documented later swap if eval shows B/32 recall is insufficient. | Phase 9 |
| D17 | Image confidence policy | ✅ **resolved** — separate **`RESKIOSK_IMAGE_SIM_THRESHOLD`** (start ~**0.26**, calibrate on the Goal-1 eval set; CLIP cosine ≠ text 0.60 scale; below floor → no image). **`RESKIOSK_IMAGE_TOP_N=3`** (primary = top-1 + up to 2 alternates). PII: **admin-curation gate** (upload already admin-only) + documented no-identifiable-residents-without-consent policy; **defer** automated face redaction; images stay local/offline. | Phases 9, 10 |
