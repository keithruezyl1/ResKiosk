---
title: What ResKiosk currently is (pre-AAIH development)
confluence_id: 229377
source: https://reskiosk.atlassian.net/wiki/spaces/SCRUM/pages/229377
parent: ResKiosk AAIH Development Home
last_updated: 2026-04-19T12:05:24.752Z
version: 1
---

# What ResKiosk currently is (pre-AAIH development)
...
1. A user starts a kiosk session and asks a question by voice or text.
2. The kiosk captures the input, applies local speech/text cleanup, and sends the query to the hub over the local network.
3. The hub translates the query to English when needed and runs normalization, intent classification, and semantic retrieval against the local knowledge base.
4. If a strong result is found, the hub formats the answer; if not, it may return a clarification or fallback response.
5. The hub translates the final answer back to the user’s language when needed and returns it to the kiosk.
6. The kiosk displays the response and plays spoken output.
7. If the interaction contains emergency intent, the kiosk can trigger an emergency alert that staff then manage through the hub and console workflow.
