...
| Phase 9 | ✅ Slice 7C DONE (S9.tune deferred) | CLIP ViT-B/32 image embedder (bundled, CPU); embeddings persisted+publish-gated; text→image retrieve_images (floor ~0.26, top-N=3, gated, deterministic); /assets/search + additive image_evidence on /query. D16/D17 resolved. 212 tests. Bias-tuning (S9.tune) deferred — D5 still open. |
| Phase 10 | ◐ hub-side done | hub: image_evidence payload + image_primary flag (RESKIOSK_IMAGE_PRIMARY_FLOOR=0.30, D17) + text-only fallback — all shipped/tested. Remaining = kiosk (Kotlin) rendering/loading/placeholders + console (React) asset-status UI + multimodal eval (real model/images) — front-end/real-data, not doable in this Python env. |
